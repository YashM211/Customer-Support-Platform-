/**
 * Custome_Support AI Chat Widget
 * 可嵌入的智能聊天组件
 */
...
function formatAssistantMessage(
  content: string,
  sources: Source[] = [],
): { content: string; references: Array<{ title: string; url: string }> } {
  if (!content) {
  ...
  for (const source of sources) {
    if (
      ...
      sourceByUrl.has(source.url)
    ) {
      continue;
    }
    sourceByUrl.set(
      source.url,
      source as Source & { type: "url"; url: string },
    );
  }
...
function buildDefaultLogoUrl(apiBase: string): string {
  ...
  try {
    return new URL("/Custome_Support-logo.png", `${apiBase}/`).toString();
  } catch {
    return "/Custome_Support-logo.png";
  }
}
...
class Custome_SupportWidget {
  ...
  private readonly VISITOR_STORAGE_KEY = "Custome_Support_visitor_id";
  ...
  constructor(config: WidgetConfig) {
    ...
    this.hasWelcomeMessageOverride =
      ...
      config.welcomeMessage.trim().length > 0;
  ...
  private detectApiBase(configuredApiBase?: string): string {
    ...
    for (const script of scripts) {
      ...
      if (!src.includes("sdk.js") && !src.includes("Custome_Support")) {
        continue;
      }
  ...
  private async loadPublicConfig() {
    ...
    try {
      const publicConfigUrl = new URL(
        `${this.config.apiBase}/api/v1/config:public`,
      );
  ...
  private showGreetingBubble() {
    ...
    bubble.className = "Custome_Support-greeting-bubble";
  ...
  private createStyles() {
    ...
    style.id = "Custome_Support-widget-styles";
  ...
  private updateUnreadBadge() {
    ...
    if (this.hasUnread) {
      if (!this.unreadBadge) {
        ...
        badge.className = "Custome_Support-unread-badge";
        badge.textContent = "1";
  ...
  private createChatWindow() {
    ...
    const safeLogoUrl = this.config.logoUrl
    ...
    this.chatWindow.innerHTML = `
      <div class="Custome_Support-header">
        <div class="Custome_Support-header-title">
          ${safeLogoUrl ? `<img src="${safeLogoUrl}" class="Custome_Support-header-logo" alt="">` : ""}
          <span>${safeTitle}</span>
        </div>
      ...
      <div class="Custome_Support-input-area">
        <input type="text" class="Custome_Support-input" placeholder="${safePlaceholder}" maxlength="2000">
  ...
  private updateMessageContent(
    ...
    element.innerHTML =
      ...
      (includeCursor
        ? '<span class="Custome_Support-stream-cursor"></span>'
        : "");
  }

  private createMessageElement(message: ChatMessage): HTMLDivElement {
    const messageDiv = document.createElement("div");
    messageDiv.className = `Custome_Support-message Custome_Support-message-${message.role}`;

    const contentDiv = document.createElement("div");
    contentDiv.className = "Custome_Support-message-content";

    if (message.role === "assistant") {
      const formattedMessage = formatAssistantMessage(
        message.content,
        message.sources,
      );
      const referenceMarkdown =
        formattedMessage.references.length > 0
          ? `\n\n**${this.getText("references")}**\n${formattedMessage.references.map((reference) => `- [${reference.title}](${reference.url})`).join("\n")}`
          : "";
      this.updateMessageContent(
        contentDiv,
        formattedMessage.content + referenceMarkdown,
      );
    } else {
    ...
    timeDiv.className = "Custome_Support-message-time";
  ...
  private showThinkingIndicator(elapsed = 0): void {
    ...
    if (!this.thinkingIndicator) {
      ...
      indicator.className = "Custome_Support-thinking";
      indicator.innerHTML = `
      ...
      this.thinkingIndicatorText = indicator.querySelector(
        "span:last-child",
  ...
  private finalizeStreamingMessage(sources: Source[] = []): void {
    ...
    const cursor = this.streamingMessage.querySelector(
      ".Custome_Support-stream-cursor",
    );
    cursor?.remove();
  ...
  private showLoading() {
    ...
    loadingDiv.className = "Custome_Support-loading";
    loadingDiv.id = "Custome_Support-loading";
   ...
   */
  private hideLoading() {
    const loading = this.chatWindow?.querySelector("#Custome_Support-loading");
    loading?.remove();
  }
  ...
  private showError(message: string) {
    ...
    errorDiv.className = "Custome_Support-error";
  ...
  private async consumeStream(response: Response): Promise<void> {
    ...
    const processEvent = (rawEvent: string) => {
      ...
      switch (eventName) {
        case "sources":
          this.currentStreamSources = Array.isArray(
        ...
        case "thinking":
          this.showThinkingIndicator(
            typeof (payload as StreamEventPayload).elapsed === "number"
        ...
        case "done": {
          ...
          if (
            typeof donePayload.message_id === "number" &&
            donePayload.message_id > this.lastMessageId
          ) {
        ...
        case "error": {
          ...
          const streamError = new Error(
            streamErrorPayload.error || "Stream failed",
          );
  ...
  private async sendMessageWithRetry(message: string): Promise<void> {
    ...
    for (let attempt = 0; attempt <= 1; attempt++) {
      ...
      try {
        ...
        const response = await fetch(
          `${this.config.apiBase}/api/v1/chat/stream`,
          {
            method: "POST",
            headers: {
        ...
        this.cleanupAfterStreamError();
        console.warn(
          `[Custome_Support Widget] Stream attempt ${attempt + 1} failed, retrying...`,
        );
  ...
  private async sendMessage(message: string) {
    ...
    try {
      ...
      console.error("[Custome_Support Widget] Error sending message:", error);
      ...
      if (error instanceof TypeError || errorText.includes("fetch")) {
        ...
        error?.name === "ORIGIN_NOT_ALLOWED" ||
        errorText.toLowerCase().includes("widget origin not allowed")
      ) {
      ...
      if (consoleHint) {
        console.error("[Custome_Support Widget]", consoleHint);
      }
  ...
  destroy() {
    ...
    const styles = document.getElementById("Custome_Support-widget-styles");
    styles?.remove();
  }
}

// 导出到全局（不使用 export default，避免 ES Module 格式）
(window as any).Custome_SupportWidget = Custome_SupportWidget;
  ...
  for (let index = scripts.length - 1; index >= 0; index -= 1) {
    ...
    try {
      ...
      if (
        getSearchParamValue(
          url.searchParams,
          AUTO_INIT_SCRIPT_PARAM_MAP.agentId,
        )
      ) {
        return script;
      }
...
function getAutoInitConfig(script: HTMLScriptElement): WidgetConfig | null {
  ...
  const agentId = getSearchParamValue(
    url.searchParams,
    AUTO_INIT_SCRIPT_PARAM_MAP.agentId,
  );
  if (!agentId) {
  ...
  const apiBase = getSearchParamValue(
    url.searchParams,
    AUTO_INIT_SCRIPT_PARAM_MAP.apiBase,
  );
  if (apiBase) {
    config.apiBase = apiBase;
  }

  const themeColor = getSearchParamValue(
    url.searchParams,
    AUTO_INIT_SCRIPT_PARAM_MAP.themeColor,
  );
  if (themeColor) {
    config.themeColor = themeColor;
  }

  const welcomeMessage = getSearchParamValue(
    url.searchParams,
    AUTO_INIT_SCRIPT_PARAM_MAP.welcomeMessage,
  );
  if (welcomeMessage) {
    config.welcomeMessage = welcomeMessage;
  }

  const language = getSearchParamValue(
    url.searchParams,
    AUTO_INIT_SCRIPT_PARAM_MAP.language,
  );
  if (language) {
    config.language = language;
  }

  const position = getSearchParamValue(
    url.searchParams,
    AUTO_INIT_SCRIPT_PARAM_MAP.position,
  );
  if (position === "left" || position === "right") {
    config.position = position;
  }

  const theme = getSearchParamValue(
    url.searchParams,
    AUTO_INIT_SCRIPT_PARAM_MAP.theme,
  );
...
(function bootstrapCustome_SupportWidget() {
  const globalWindow = window as Window &
    typeof globalThis & {
      __Custome_SupportWidgetAutoInitScheduled?: boolean;
    };
  ...
  if (globalWindow.__Custome_SupportWidgetAutoInitScheduled) {
    return;
  }
  globalWindow.__Custome_SupportWidgetAutoInitScheduled = true;

  const start = () => {
    void new Custome_SupportWidget(config).init();
  };
