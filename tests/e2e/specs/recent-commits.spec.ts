...
test.describe('Recent commit regressions', () => {
  ...
  test('widget renderer does not execute malicious markdown-like content in welcome message', async ({ page, request }) => {
    ...
    await page.evaluate((currentAgentId) => {
      const WidgetClass = (window as any).Custome_SupportWidget;
      const widget = new WidgetClass({ agentId: currentAgentId, apiBase: 'http://localhost:8000' });
      return widget.init();
    }, agent.id);
    await expect(page.locator('#Custome_Support-widget-button')).toBeVisible({ timeout: 10_000 });
    await page.locator('#Custome_Support-widget-button').click();
    await page.waitForTimeout(1_000);
    expect(dialogs).toEqual([]);

    const dangerousNodes = await page.locator('#Custome_Support-widget-container a[href^="javascript:"], #Custome_Support-widget-container script, #Custome_Support-widget-container img[onerror]').count();
